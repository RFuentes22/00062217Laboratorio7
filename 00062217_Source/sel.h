...
Matrix createLocalK(int e,mesh &m){
    ...
    // [ A+K  E-L ]
    // [  D   0 ]
    Matrix matrixA,matrixK,matrixE,matrixL,matrixD;
    ...
    productRealMatrix(thao*J/(24*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,Beta,2,2,6),6,2,6),matrixA);

    //Preparando matrixK (En clase conocida simplemente como K)
    kappa = m.getParameter(KAPPA);
    Ae = calculateLocalArea(e,m);
    transpose(Alpha,Alphat);
    transpose(Beta,Betat);
    productRealMatrix(kappa*Ae/(D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,Beta,2,2,6),2,2,6),6,2,6),matrixK);

    //Preparando matrixE 
    calculateBPrima(BPrima);
    productRealMatrix(2*kappa*J/(18*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixE);
    ...
    ubicarSubMatriz(K,0,5,6,8,restMatrix(matrixE,matrixL,6,3));
